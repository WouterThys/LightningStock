 ...
 *          MACRO FUNCTIONS
 ******************************************************************************/
#define SPI_Write(data) D_SPI_WriteByte(SPI_MODULE_1, data)
 ...
 *          VARIABLES
 ******************************************************************************/
uint16_t i; // For loops
NRF_Status_t NRF_Status;
NRF_Data_t NRF_DataWrite;
NRF_Data_t NRF_DataRead;
bool writeBusy, readFlag, writeFlag;
uint16_t waitCnt, resendCnt; 
 ...
 *          LOCAL FUNCTIONS
*******************************************************************************/
void    NRF_WriteRegister(uint8_t reg, uint8_t value); // Write to a register
uint8_t NRF_ReadRegister(uint8_t reg); // Read from a register
void    NRF_WriteData(void); // Write data
void    NRF_ReadData(void); // Read data
void    NRF_Configure(void); // Configure the registers of the NRF module
void    NRF_WaitForLink(void); // Wait for a link between the modules
...
void NRF_WriteData() {
    NRF_WriteRegister(NRF_TXBUF0_Adr, NRF_DataWrite.command);
    NRF_WriteRegister(NRF_TXBUF1_Adr, NRF_DataWrite.data);
    NRF_WriteRegister(NRF_TXBUF2_Adr, NRF_DataWrite.command + NRF_DataWrite.data);
...
void NRF_ReadData() {
    NRF_DataRead.command = NRF_ReadRegister(NRF_RXBUF0_Adr);
    NRF_DataRead.data = NRF_ReadRegister(NRF_RXBUF1_Adr);
    NRF_DataRead.crc = NRF_ReadRegister(NRF_RXBUF2_Adr);
}
...
void C_NRF_Init() {
    ...
    // Initialize status
    NRF_Status.STA = NRF_STATUS_Initialize;
    ...
    switch(interruptSource) {
        ...
        case NRF_STATUS_IntDR:
            ...
            if(writeBusy){
                ...
                if(NRF_DataRead.crc == (NRF_DataRead.command + NRF_DataRead.data)) {
                    NRF_DataWrite.command = CRC_OK;
                    C_DEBUG_WriteRegister(REG_CUSTOM, NRF_DataRead.data);
                } else {
                    NRF_DataWrite.command = CRC_NOK;
                }
                NRF_WriteData();
            }
...
void C_NRF_WriteData(NRF_Data_t data) {
    ...
    while(writeBusy) {
        ...
        switch(WFsm.State) {
            ...
            case WFSM_SCheck:
                if(NRF_DataRead.command == CRC_OK) {
                    WFsm.State = WFSM_SSetup;
                    writeBusy = false;
                    readFlag = false;
                } else {
        ...
        switch(WFsm.State) {
            case WFSM_SSetup:
                waitCnt = 0;
                resendCnt = 0;
                NRF_DataWrite.command = data.command;
                NRF_DataWrite.data = data.data; 
                break;
